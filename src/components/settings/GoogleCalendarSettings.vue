  ...
  <div class="gcal-settings">
    ...
    <div class="gcal-header">
      <div class="header-left">
        <div class="gcal-icon-wrapper">
          <GoogleCalendarIcon />
        </div>
        <div>
          <h3 class="gcal-title">Google Calendar Integration</h3>
          <p class="gcal-subtitle">Sync calendar events as structured Markdown notes</p>
        </div>
      </div>
    ...
    <div class="config-section">
      ...
      <div v-if="showSetupGuide" class="setup-guide">
        <ol class="guide-steps">
          ...
          <li>
            Navigate to <strong>APIs & Services &gt; Library</strong>. Search for and enable the
            <a href="#" @click.prevent="openExternal('https://console.cloud.google.com/apis/library/calendar-json.googleapis.com')">
              <strong>Google Calendar API</strong></a>.
          </li>
          <li>
          ...
          <li>
            ...
            Click <strong>Add new</strong>, paste the service account's <strong>Client ID</strong>, and add this scope:
      ...
      <div class="field-group">
        ...
        <input
          ...
          class="text-input"
        />
        <span class="field-hint">A Google Workspace user email that the service account will impersonate.</span>
      </div>
    ...
    <div class="config-section" v-if="availableCalendars.length > 0">
      <h4 class="section-label">
        <ion-icon :icon="calendarOutline" />
        Calendars
      </h4>
      <p class="section-hint">Select which calendars to sync. If none are selected, only the primary calendar is synced.</p>
      ...
      <div class="calendar-list">
        ...
        >
          <input
            type="checkbox"
            :value="cal.id"
            v-model="localSettings.syncSettings.selectedCalendarIds"
    ...
    <div class="config-section">
      ...
      <div class="field-row">
        <div class="field-group">
          ...
          <select v-model.number="localSettings.syncSettings.pastDays" class="select-input">
        ...
        <div class="field-group">
          ...
          <select v-model.number="localSettings.syncSettings.futureDays" class="select-input">
            <option :value="0">Today only</option>
            <option :value="1">1 day</option>
            <option :value="3">3 days</option>
            <option :value="7">7 days</option>
    ...
    <div class="config-section" v-if="isConnected">
      ...
      <div class="status-grid">
        ...
        <div class="status-item">
          ...
          <span class="status-value">
            {{ localSettings.syncSettings.syncedEventIds.length }}
          </span>
        </div>
      </div>
...
import type { GoogleCalendarSettings, GoogleCalendarSyncEvent, GoogleCalendarListEntry, Project } from '@/types';
...
const localSettings = ref<GoogleCalendarSettings>({ ...DEFAULT_GOOGLE_CALENDAR_SETTINGS });
const projects = ref<Project[]>([]);
const availableCalendars = ref<GoogleCalendarListEntry[]>([]);
const showSetupGuide = ref(false);
const testing = ref(false);
const testResult = ref<{ success: boolean; message: string } | null>(null);
const syncLog = ref<GoogleCalendarSyncEvent[]>([]);
...
onMounted(async () => {
  localSettings.value = loadGoogleCalendarSettings();
  ...
  unsubscribeSyncEvent = onSyncEvent((event: GoogleCalendarSyncEvent) => {
...
async function handleTestConnection() {
  ...
  try {
    saveGoogleCalendarSettings(localSettings.value);
...
async function handleDeactivate() {
  ...
  const toast = await toastController.create({
    message: 'Google Calendar integration deactivated',
    duration: 2000,
    color: 'warning',
    position: 'top',
  });
