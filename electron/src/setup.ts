...
export class ElectronCapacitorApp {
  ...
  async init(): Promise<void> {
    ...
    // On macOS the menu bar is part of the OS chrome; on Windows/Linux remove it
    // to avoid the unwanted in-window system menu.
...
export function setupContentSecurityPolicy(customScheme: string): void {
  session.defaultSession.webRequest.onHeadersReceived((details, callback) => {
    // Define allowed external sources for the app
    const connectSrc = [
      "'self'",
      `${customScheme}://*`,
      // OpenAI API
      ...
      // Local services (Ollama, SearXNG, etc.)
      'http://localhost:*',
      'http://127.0.0.1:*',
      // DuckDuckGo
      ...
      // Allow any HTTPS for web research fetching
      'https://*',
      'http://*',
      // Blob URLs for DuckDB WASM workers
      'blob:',
    ].join(' ');
